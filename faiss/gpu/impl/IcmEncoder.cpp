...
void IcmEncoderImpl::computeUnaryTerms(
    ...
    for (int m = 0; m < M; m++) {
        ...
        hipLaunchKernelGGL(runNormAddition, n, K, 0, stream, uPtr, nPtr, K);
    }
}
...
void IcmEncoderImpl::encode(
    ...
    hipLaunchKernelGGL(runEvaluation, n, dims, evaluateSmem, stream, 
            x.data(),
            codebooks.data(),
            codes.data(),
            bestObjs.data(),
    ...
    for (int i = 0; i < ilsIters; i++) {
        hipLaunchKernelGGL(runCodesPerturbation, numBlocks, blockSize, 0, stream, 
                gen(), codes.data(), n, M, K, nperts);

        // perform icm encoding
        for (int j = 0; j < icmIters; j++) {
            for (int m = 0; m < M; m++) {
                hipLaunchKernelGGL(runIcmEncodeStep, n, K, encodeSmem, stream, 
                        uterm[m].data(),
                        bterm[m].data(),
                        codes.data(),
                        M,
        ...
        hipLaunchKernelGGL(runCodesSelection, numBlocks, blockSize, 0, stream, 
                bestCodes.data(),
                bestObjs.data(),
                codes.data(),
                objs.data(),
