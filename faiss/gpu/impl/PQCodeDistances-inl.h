...
void runPQResidualVector(
        ...
        Tensor<float, 4, true>& residual,
        bool l2Residual,
<<<<<<< HEAD:faiss/gpu/impl/PQCodeDistances-inl.cuh
        cudaStream_t stream) {
    // blockDim.y is limited by nprobe
=======
        hipStream_t stream) {
>>>>>>> port_to_rocm/v1.7.3:faiss/gpu/impl/PQCodeDistances-inl.h
    auto grid = dim3(coarseIndices.getSize(0), coarseIndices.getSize(1));
    auto block = dim3(
            std::min(queries.getSize(1), (idx_t)getMaxThreadsCurrentDevice()));

    if (l2Residual) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(pqResidualVector<CentroidT, true>), grid, block, 0, stream, 
                queries,
                coarseCentroids,
                coarseIndices,
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(pqResidualVector<CentroidT, false>), grid, block, 0, stream, 
                queries,
                coarseCentroids,
                coarseIndices,
                pqCentroids.getSize(1),
...
>>>>>>> port_to_rocm/v1.7.3:faiss/gpu/impl/PQCodeDistances-inl.h
    ...
    hipLaunchKernelGGL(pqDistanceIPCorrection, grid, block, 0, stream, 
