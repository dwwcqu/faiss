...
#include <assert.h>
<<<<<<< HEAD:faiss/gpu/utils/Tensor.cuh
#include <cuda.h>
...
>>>>>>> port_to_rocm/v1.7.3:faiss/gpu/utils/Tensor.h
...
class Tensor {
   public:
    ...
    __host__ void copyFrom(const std::vector<T>& v, hipStream_t stream);

    /// Copies ourselves into a flattened (1D) std::vector, using the given
    /// stream
    __host__ std::vector<T> copyToVector(hipStream_t stream);
